...
// import { setFaviurate } from "../Components/Action";
...
const History_Quizqestion = ()=>{
  const Username = useSelector((state) => state.User.username);
    ...
    const handleCategory = (category,subcategory) => {
        ...
        const Addtofav = (category, subcategory) => {
          ...
          axios.post("http://localhost:3001/Addtofav", formdata)
              .then((response) => {
                  if (response) {
                      alert(response.data.message);
                      // dispatch(setFaviurate({ category, subcategory }))
                  } else {
     ...
     <div className="Quiz_Heding">
      ...
      <div className="Quiz_cards_flex">
        ...
        <p>Question:<span>10</span></p>
         <button className="Takequiz"  onClick={() => handleCategory('History', 'Modern Era')}>Take Quiz</button> <button onClick={()=>{Addtofav('History', 'Modern Era')}} className="AddtoFav_button"><FontAwesomeIcon icon={faStar} className="starIcon" /></button>
        </div> 
        </div>
        ...
        <p>Question:<span>10</span></p>
        <button className="Takequiz" onClick={() => handleCategory('History', 'Medieval History')}>Take Quiz</button> <button onClick={()=>{Addtofav('History', 'edieval History')}} className="AddtoFav_button"><FontAwesomeIcon icon={faStar} className="starIcon" /></button>
        </div>
